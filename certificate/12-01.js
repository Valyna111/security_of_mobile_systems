...
app.use(bodyParser.urlencoded({extended: true}));//вложенные структуры
app.use(session({secret: 'super secret key'}));
app.use(passport.initialize());
app.use(passport.session());
passport.serializeUser((user, done) => done(null, user));//сохранение в сессии
passport.deserializeUser((user, done) => done(null, user));//восстановление из сессии
